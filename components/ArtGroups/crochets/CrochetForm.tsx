...
interface CrochetFormProps {
  initialData?: CrochetType | null; // Must have "?" to make it optional
}

const CrochetForm: React.FC<CrochetFormProps> = ({ initialData }) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      setLoading(true);
      const url = initialData ? `/api/crochets/${initialData._id}` : "/api/crochets";
      const res = await fetch(url, {
        method: "POST",
        body: JSON.stringify(values),
      });
      if (res.ok) {
        toast.success(`Crochet ${initialData ? "updated" : "created"}`);
        router.push("/crochets");
      } else {
    ...
    <div className="p-10">
      {initialData ? (
        <div className="flex items-center justify-between">
          <p className="text-heading2-bold">Edit Crochet</p>
          <Delete id={initialData._id} item="crochet" />
        </div>
      ) : (
        <p className="text-heading2-bold">Create Crochet</p>
      )}
