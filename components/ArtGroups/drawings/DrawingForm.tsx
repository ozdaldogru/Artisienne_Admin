...
interface DrawingFormProps {
  initialData?: DrawingType | null; // Must have "?" to make it optional
}

const DrawingForm: React.FC<DrawingFormProps> = ({ initialData }) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      setLoading(true);
      const url = initialData ? `/api/drawings/${initialData._id}` : "/api/drawings";
      const res = await fetch(url, {
        method: "POST",
        body: JSON.stringify(values),
      });
      if (res.ok) {
        toast.success(`Drawing ${initialData ? "updated" : "created"}`);
    ...
    <div className="p-10">
      {initialData ? (
        <div className="flex items-center justify-between">
          <p className="text-heading2-bold">Edit Drawing</p>
          <Delete id={initialData._id} item="drawing" />
        </div>
      ) : (
        <p className="text-heading2-bold">Create Drawing</p>
      )}
