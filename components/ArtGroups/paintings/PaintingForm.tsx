...
interface PaintingFormProps {
  initialData?: PaintingType | null; // Must have "?" to make it optional
}

const PaintingForm: React.FC<PaintingFormProps> = ({ initialData }) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      setLoading(true);
      const url = initialData ? `/api/paintings/${initialData._id}` : "/api/paintings";
      const res = await fetch(url, {
        method: "POST",
        body: JSON.stringify(values),
      });
      if (res.ok) {
        toast.success(`Painting ${initialData ? "updated" : "created"}`);
    ...
    <div className="p-10">
      {initialData ? (
        <div className="flex items-center justify-between">
          <p className="text-heading2-bold">Edit Painting</p>
          <Delete id={initialData._id} item="painting" />
        </div>
      ) : (
        <p className="text-heading2-bold">Create Painting</p>
      )}
