...
interface JewelryFormProps {
  initialData?: JewelryType | null; // Must have "?" to make it optional
}

const JewelryForm: React.FC<JewelryFormProps> = ({ initialData }) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      setLoading(true);
      const url = initialData ? `/api/jewelries/${initialData._id}` : "/api/jewelries";
      const res = await fetch(url, {
        method: "POST",
        body: JSON.stringify(values),
      });
      if (res.ok) {
        toast.success(`Jewelry ${initialData ? "updated" : "created"}`);
        router.push("/jewelries");
      } else {
    ...
    <div className="p-10">
      {initialData ? (
        <div className="flex items-center justify-between">
          <p className="text-heading2-bold">Edit Jewelry</p>
          <Delete id={initialData._id} item="jewelry" />
        </div>
      ) : (
        <p className="text-heading2-bold">Create Jewelry</p>
      )}
