...
interface WearableFormProps {
  initialData?: WearableType | null; // Must have "?" to make it optional
}

const WearableForm: React.FC<WearableFormProps> = ({ initialData }) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      setLoading(true);
      const url = initialData ? `/api/wearables/${initialData._id}` : "/api/wearables";
      const res = await fetch(url, {
        method: "POST",
        body: JSON.stringify(values),
      });
      if (res.ok) {
        toast.success(`Wearable ${initialData ? "updated" : "created"}`);
    ...
    <div className="p-10">
      {initialData ? (
        <div className="flex items-center justify-between">
          <p className="text-heading2-bold">Edit Wearable</p>
          <Delete id={initialData._id} item="wearable" />
        </div>
      ) : (
        <p className="text-heading2-bold">Create Wearable</p>
      )}
